...
class VippsApi {
    ...
    private function get_access_token_from_vipps() { 
        ...
        try {
            ...
            $this->log(__("Could not get Vipps access token",'vipps-for-woocommerce') .' '. $e->getMessage());
    ...
    public function login_request($returnurl,$authtoken,$requestid) {
        ...
        if (!$subkey) {
            throw new VippsAPIConfigurationException(__('The Vipps gateway is not correctly configured.','vipps-for-woocommerce'));
            $this->log(__('The Vipps gateway is not correctly configured.','vipps-for-woocommerce'),'error');
        }
        if (!$merch) {
            throw new VippsAPIConfigurationException(__('The Vipps gateway is not correctly configured.','vipps-for-woocommerce'));
            $this->log(__('The Vipps gateway is not correctly configured.','vipps-for-woocommerce'),'error');
    ...
    public function initiate_payment($phone,$order,$returnurl,$authtoken,$requestid) {
        ...
        $transaction['transactionText'] = __('Confirm your order from','vipps-for-woocommerce') . ' ' . home_url(); 
    ...
    public function capture_payment($order,$requestid=1,$amount=0) {
        ...
        $transaction['transactionText'] = __('Order capture for order','vipps-for-woocommerce') . ' ' . $orderid . ' ' . home_url(); 
    ...
    public function cancel_payment($order,$requestid=1) {
        ...
        $transaction = array();
        $transaction['transactionText'] = __('Order cancel for order','vipps-for-woocommerce') . ' ' . $orderid . ' ';
    ...
    public function refund_payment($order,$requestid=1,$amount=0,$cents=false) {
        ...
        $transaction['transactionText'] = __('Refund for order','vipps-for-woocommerce') . ' ' . $orderid;
    ...
    private function handle_http_response ($response, $headers, $content) {
        ...
        if (!$response) {
            $msg = __('No response from Vipps', 'vipps-for-woocommerce');
