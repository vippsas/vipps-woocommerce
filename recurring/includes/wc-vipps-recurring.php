...
class WC_Vipps_Recurring {
	...
	public function init() {
		...
		// Schedule cleaning up subscriptions that are marked for deletion
		if ( ! wp_next_scheduled( 'woocommerce_vipps_recurring_check_subscriptions_marked_for_deletion' ) ) {
			wp_schedule_event( time(), 'hourly', 'woocommerce_vipps_recurring_check_subscriptions_marked_for_deletion' );
		}

		add_action( 'woocommerce_vipps_recurring_check_subscriptions_marked_for_deletion', [
			$this,
			'check_subscriptions_marked_for_deletion'
		] );

		add_action( 'woocommerce_vipps_recurring_delete_pending_subscription', [
			$this->gateway(),
...
//				}
	...
	public function gateway_should_be_active( array $methods = [] ) {
		// Disable our gateway if the standard Vipps payment method is present.
	...
	public function check_subscriptions_marked_for_deletion() {
		$options = [
			'limit'          => 25,
			'payment_method' => $this->gateway()->id,
		];
		...
		WC_Vipps_Recurring_Logger::log( sprintf( "Running check_subscriptions_marked_for_deletion for %s subscriptions", count( $subscriptions ) ) );
		...
		foreach ( $subscriptions as $subscription ) {
			...
			$this->gateway()->check_charge_status( $latest_order_id );
			do_action( 'wc_vipps_recurring_after_cron_check_order_status', $latest_order_id );

			// Hydrate objects
			$latest_order = wc_get_order( $latest_order_id );
			$subscription = wcs_get_subscription( $subscription_id );

			// If this subscription has been manually updated in the mean-time, we no longer want to delete it.
