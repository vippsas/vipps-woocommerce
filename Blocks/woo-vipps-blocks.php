...
/**
 * Init hooks and inline script for the block buy-now. LP 14.11.2024.
 *
 * @return void
 */
function vipps_buy_now_block_hooks() {
    add_action('init', function () {
        register_block_type(__DIR__ . '/dist/buy-now');
    });

    // // Inject block config variables to buy-now editor script. LP 15.11.2024
    // add_action('enqueue_block_editor_assets', function () {
    //     $vipps = Vipps::instance();
    //     $variants = $variants = [
    //         ['label' => __('White', 'woo-vipps'), 'value' => 'white'],
    ...
    //     wp_add_inline_script('woo-vipps-buy-now-editor-script',
    //         'const injectedVippsBadgeBlockConfig = ' . json_encode($block_config),
    //         'before');
    // });
}
